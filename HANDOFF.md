...
- **Incorrect Base Path**: The build originally set base to `/pubsite/` in `vite.config.js`. Because the GitHub repository is named `atauln-pubsite`, the deployed assets returned 404 HTML pages which triggered MIME type execution blocks in the browser. Changing the base path to `'/atauln-pubsite/'` resolved this.
